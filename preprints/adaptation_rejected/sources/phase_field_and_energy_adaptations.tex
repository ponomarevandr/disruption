...
\end{align}
Числовые константы $\tol_1$ и $\tol_2$ подбираются эмпирически. Здесь и далее символами $[\; \cdot \;]_h$ и $[\; \cdot \;]_j^k$ обозначаются сеточные функции, а именно разностные производные и приращения ($j$, $k$ -- индексы на сетке, целые либо полуцелые).
...
Общность описанных двух подходов и, возможно, ключ к их интуитивному пониманию заключается в следующем. Для адаптации по фазовому полю рассмотрим норму приращения $\phi^{k + 1} - \phi^k$:
\[
	\left\| \phi^{k + 1} - \phi^k \right\|_C = \widetilde{\tau}_1^k \cdot \left\| \left[ \partt{\phi} \right]^{k + 1/2} \right\|_C = \tol_1 \left\| \left[ \partt{\phi} \right]^{k + 1/2} \right\|_C^{-1} \cdot \left\| \left[ \partt{\phi} \right]^{k + 1/2} \right\|_C = \tol_1.
\]
...
\[
	\cfrac{d \Pi}{dt} = -\cfrac{1}{m} \varint\limits_{\Omega} \left( \partt{\phi} \right)^2 d \vx,
\]
...
\begin{multline*}
	\left\| \phi^{k + 1} - \phi^k \right\|_2 = \widetilde{\tau}_2^k \cdot \left\| \left[ \partt{\phi} \right]^{k + 1/2} \right\|_2 \approx \tol_2 \cdot m \left\| \left[ \partt{\phi} \right]^{k - 1/2} \right\|_2^{-2} \cdot \left\| \left[ \partt{\phi} \right]^{k + 1/2} \right\|_2 \approx \\ \approx \tol_2 \cdot m \cdot \left\| \left[ \partt{\phi} \right]_h \right\|_2^{-1}.
\end{multline*}
