...
Внешние микросилы $\gamma$ и приток $r$ тепловой энергии извне будем считать нулевыми: $\gamma = 0$, $r = 0$.


\section{Определение функций свойств среды}

Для замыкания уравений модели остается определить функции $\epsilon$, $\sigma$, $\beta$, $k$ переменных $\theta$ и $\phi$, выражающие зависимость свойств среды от температуры и фазового поля, то есть связь этих свойств с фазой вещества; а также составляющую $\psi^{(0)}$ плотности свободной энергии.
...
\end{align}
\label{eq:electrical_properties}
\end{subequations}
...
Положим $\beta(\theta, \phi) = 1 / m = \Const$, где $m$ задает скорость изменения $\phi$ под действием единичной обобщенной силы и называется подвижностью. Пусть коэффициент теплопроводности $k(\theta, \phi) = k = \Const$. Вообще же отметим, что произвольные нетривиальные зависимости $\beta(\theta, \phi)$ и $k(\theta, \phi)$ не влияют на дальнейшие выкладки.
...
Для вывода формулы $\psi^T$ будем считать, что теплоемкость среды $c = -\theta \hat{\psi}_{\theta \theta} = -\theta \psi^T_{\theta \theta} \hm = c(\phi)$, то есть не зависит от температуры. Тогда, интегрируя, получим
\[
    \psi^T = c(\phi) \theta (1 - \ln \theta) - \widetilde{C}(\phi) \theta,
\]
...
\end{align}
\label{eq:entropy_thermal_free_energy}
\end{subequations}

Отметим, что величина $l = \sqrt{2 \lambda / \Gamma_V}$ есть масштабный коэффициент характерной толщины диффузной границы. Переобозначим $\Gamma_V = \Gamma / l^2$, $\lambda = \Gamma / 2 $; здесь параметр $\Gamma$ имеет смысл характерной энергии образования канала пробоя единичной длины (подразумевается <<площадь сечения>> $\approx l^2$).