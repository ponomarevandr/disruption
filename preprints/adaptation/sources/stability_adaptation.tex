...
\]
Полученный ряд есть $S_{j + 1}$, домноженный на $\delta^{j / 3}$ и ограниченную функцию, следовательно, он сходится на $[0, \delta^{-1/3}]$ равномерно. Так как и сам $S_j$, и ряд из его производных равномерно сходятся, то $S_j$ можно продифференцировать почленно, то есть
\[
