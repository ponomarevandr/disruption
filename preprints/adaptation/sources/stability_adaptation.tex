...
\end{equation}
где $1 \geqslant C \approx 1.1 / 2$ -- константа, выбранная, во-первых, для создания <<запаса>> в оценке, во-вторых, для удобства формульной записи.
...
\begin{equation}
	\widetilde{\tau}_3^k = \cfrac{\tol_3}{m \cdot \max\limits_{j = 0}^M |F''(\phi_j^k)|}.
	\label{sch:time_step_stability_raw}
\end{equation}
Этот метод будем условно называть методом адаптации по устойчивости.

Идея описанного подхода подразумевает, что для корректной работы схемы должно быть достаточно $\tol_3 = 1$, позволяя отказаться от подбора значения. При большей желаемой точности расчета можно провести подбор $\tol_3 < 1$.
...
\[
	\cfrac{\delta \epsilon(z \delta^{1/3})}{\epsilon_0} = \cfrac{1}{4 z^3 - 3 z^4 \delta^{1/3} + 1}.
\]
...
Производные $\delta \epsilon$ порядка $k$ по $z$ и по $\phi$ легко выражаются друг через друга:
\[
	\cfrac{d^k}{d z^k} \left[ \delta \epsilon(z \delta^{1/3}) \right] = \delta^{1 + k/3} \cdot \epsilon^{(k)}(z \delta^{1/3}),
\]
где $\epsilon^{(k)}$ обозначена $d^k \epsilon / d \phi^k$.

Очевидное утверждение о поточечной сходимости $\delta \epsilon(z \delta^{1/3}) / \epsilon_0$ может быть значительно усилено.
...
\begin{proposition}
	...
	\[
		\cfrac{d^k}{d z^k} \left[ \cfrac{\delta \epsilon(z \delta^{1/3})}{\epsilon_0} \right] \rightrightarrows \cfrac{d^k}{d z^k} \left[ \cfrac{1}{1 + 4 z^3} \right]
	\]
	при $\delta \to +0$.
\end{proposition}
\begin{proof}
	Обозначим исследуемую функцию $g(z) = \delta \epsilon(z \delta^{1/3}) / \epsilon_0$.
	...
	\]
	В этих обозначениях $g(z) = S_0(z) \cdot 1 / (1 + 4 z^3)$.
...
Утверждение \ref{prop:convergence_uniform} позволяют записать приближенные представления
\[
	\cfrac{d^k \epsilon(\phi)}{d \phi^k} \approx \epsilon_0 \delta^{-1 - k / 3} \cdot \cfrac{d^k}{d z^k} \left[ \cfrac{1}{1 + 4 z^3} \right] \bigg|_{z = \delta^{-1/3} \phi}.
\]
Отсюда становится совершенно ясным описанное в работе \cite{ponomarev_stability} убывание корней $\epsilon''$ с порядком $\delta^{1/3}$ и порядок $\delta^{-5/3}$ модулей экстремумов $\epsilon''$.

Функция $\epsilon(\phi)$ на отрезке $[0, 1]$ монотонно убывает; ее производная унимодальна: при увеличении $\phi$ сначала убывает от значения $0$, затем возрастает к $0$; вторая производная имеет три промежутка роста: убывает, затем возрастает, затем убывает. $\epsilon''$ вблизи $0$ меняется очень быстро, достигает больших по модулю значений и всецело определяет поведение $F''$. $\epsilon''(\phi)$ имеет ноль $\phi_0 \sim 0.5 \cdot \delta^{1/3}$, а также локальный минимум и локальный максимум в точках
\[
...
\begin{multline*}
	...
	= \cfrac{12t (16 z^3 - 30 \delta^{1/3} z^4 + 15 \delta^{2/3} z^5 + 3z \delta^{1/3} - 2)}{(4 z^3 - 3 \delta^{1/3} z^4 + 1)^3}.
\end{multline*}
Поведение функции вблизи $0$ определяется слагаемым $-2 \cdot 12z$. Поменяем его знак и получим функцию
\begin{gather}
	\widetilde{G}(z) = \cfrac{12t (16 z^3 - 30 \delta^{1/3} z^4 + 15 \delta^{2/3} z^5 + 3z \delta^{1/3} + 2)}{(4 z^3 - 3 \delta^{1/3} z^4 + 1)^3};
	\nonumber \\
