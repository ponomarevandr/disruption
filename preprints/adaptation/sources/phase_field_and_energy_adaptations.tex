...
\[
	\left\| \left[ \partt{\phi} \right]_h^{k + 1/2} \right\|_C = \max\limits_{j = 0}^M \left| \left[ \partt{\phi} \right]_j^{k + 1/2} \right|.
\]
...
\begin{gather*}
	\pi_j^k = F(\phi_j^k) + \cfrac{\Gamma}{4} \left( \left[ \partt{\phi} \right]_j^k \right)^2, \qquad \left[ \partt{\phi} \right]_j^k = \begin{cases}
		...
		\cfrac{\phi_{j + 1}^k - \phi_{j - 1}^k}{2 \tau} & \text{при } j = \overline{1, M - 1}; \\
		\cfrac{\phi_M^k - \phi_{M - 1}^k}{\tau} & \text{при } j = M;
	\end{cases} \\
