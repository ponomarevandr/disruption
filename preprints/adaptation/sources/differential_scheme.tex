...
\begin{align}
	...
	\label{sch:borders}
\end{align}
Схема четырехточечная явная, на регулярной сетке с временным шагом~$\tau$ и пространственным шагом~$h$, с числом узлов пространственной сетки $M \hm = W / h \in \Natural$. Координаты узлов сетки -- $(jh, k \tau)$, $j \hm = \overline{0, M}$, $k \in \Natural_0$. $\phi_j^k$ -- значение сеточной функции $\phi$ в узле $(jh, k \tau)$.
