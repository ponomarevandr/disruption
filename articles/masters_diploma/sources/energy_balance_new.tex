...
На <<боковых>> гранях $\partial \Omega$ зададим однородное условие Неймана для электрического потенциала:
\begin{equation}
    \partvn{\Phi} \bigg|_{ \{ x = 0, L \} \cup \{ z = 0, D \} } = 0;
\end{equation}
а также условие Дирихле либо Неймана для фазового поля:
\begin{equation}
    \phi|_{ \{ x = 0, L \} \cup \{ z = 0, D \} } = 1 \quad \text{либо} \quad \partvn{\phi} \bigg|_{ \{ x = 0, L \} \cup \{ z = 0, D \} } = 0.
\end{equation}
На верхней и нижней грани считаем заданным однородное условие Неймана для фазового поля:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:charge}
\end{equation}
Интегрируя уравнение \eqref{eq:electrostatic_c}, по формуле Гаусса--Остроградского получаем
\begin{equation}
...
\begin{equation}
    \Div(\zeta \vect{V}) = \zeta \Div \vect{V} + (\nabla \zeta, \vect{V}),
...
\[
    2 W_E = \intOmega 2 w_E d \vx = \intpartOmega \Phi \epsilon \cdot (\nabla \Phi, \vn) dS - \intOmega \Phi \Div(\epsilon \nabla \Phi) d \vx = -\Phi^+ q^- + \intOmega \Phi \rho d \vx.
\]
\end{proof}

\begin{proposition}
\label{prop:energy_electrical_deriv_form}
...
\begin{align}
    -q^- \cfrac{d \Phi^+}{dt} + \intOmega \rho \partt{\Phi} d \vx & = \intOmega \epsilon \cdot \left( \nabla \Phi, \nabla \partt{\Phi} \right) d \vx,
    \label{eq:energy_electrical_deriv_term_a} \\
    -\Phi^+ \cfrac{d q^-}{dt} + \intOmega \Phi \partt{\rho} d \vx & = \intOmega \epsilon \cdot \left( \nabla \Phi, \nabla \partt{\Phi} \right) d \vx + \intOmega \partt{\epsilon} (\nabla \Phi, \nabla \Phi) d \vx.
...
\end{proposition}
\begin{proof}
Из правой части формулы \eqref{eq:energy_electrical_deriv} вычтем и прибавим части ра\forcehyphenation венств~\eqref{eq:energy_electrical_deriv_term_a} и~\eqref{eq:energy_electrical_deriv_term_b}, получим соответственно выражение \eqref{eq:energy_electrical_balance_a} и
\begin{equation}
    \cfrac{d W_E}{dt} = -\half \intOmega \partt{\epsilon} (\nabla \Phi, \nabla \Phi) d \vx - \Phi^+ \cfrac{d q^-}{dt} + \intOmega \Phi \partt{\rho} d \vx.
    \label{tmp:energy_electrical_deriv}
\end{equation}
Преобразуем последнее слагаемое. Выразим производную плотности заряда $\rho$ из уравнения \eqref{eq:electrostatic_b} и далее по дифференциальному тождеству \eqref{eq:identity_gradients_product} имеем
\begin{multline*}
...
При динамике фазового поля, определяемой уравнением \eqref{eq:phi}, справедливо следующее уравнение баланса свободной энергии $\Psi$ системы:
\begin{equation}
...
\end{multline*}
Выделяя под интегралом правую часть уравнения \eqref{eq:phi} динамики фазового поля, получаем выражение \eqref{eq:energy_free_balance}.
