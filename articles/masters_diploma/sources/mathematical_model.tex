...
На <<боковых>> гранях $\partial \Omega$ зададим однородное условие Неймана на электрический потенциал: $\partflvn{\Phi}|_{ \{ x = 0, L \} \cup \{ z = 0, D \} } = 0$; а также условие Дирихле либо Неймана на фазовое поле: $\phi|_{ \{ x = 0, L \} \cup \{ z = 0, D \} } = 1$ либо $\partflvn{\phi}|_{ \{ x = 0, L \} \cup \{ z = 0, D \} } = 0$. На верхней и нижней грани считаем заданным однородное условие Неймана на фазовое поле.

Так как пластины из идеального проводника, то $\Phi|_{y = 0} = \Phi^+(t)$, $\Phi|_{y = H} = \Phi^-(t)$, где $\Phi^+$ и $\Phi^-$ -- функции времени. Потенциал на обкладках может меняться как из-за притока заряда извне, так и за счет изменения сопротивления образца между ними.
...
\begin{gather}
	...
	\partt{q^+} = \int\limits_{y = 0} \left( \partt{\epsilon} \party{\Phi} + \epsilon \partt{} \party{\Phi} \right) dS.
...
\end{equation}
Преобразуя пер{}вое слагаемое с учетом \eqref{eq:tangent_temporal_rho}, \eqref{eq:tangent_temporal_charge} тем же образом, что для $\partial W_E$ в доказательстве утверждения \ref{prop:minimization}, имеем
\[
...
\]
В этом случае уравнение \eqref{eq:energy_electrical_balance} принимает вид
\[
